<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    ...
    vistas.ViewProducto,datos.Dt_TipoProducto,datos.Dt_Producto,entidades.Producto, entidades.Rol,vistas.ViewRolUsuario, vistas.ViewRolOpcion, datos.Dt_Rol,datos.Dt_RolOpcion, java.util.*;"%>
<%
...
<body id="page-top">
        ...
        <!-- Menus -->
  		 <jsp:include page="adminMenus.jsp" />    
                ...
                <div class="container-fluid">
              				  <%
                            	String prID = "";
              					prID = request.getParameter("idP")==null?"0":request.getParameter("idP");
														
								Producto pr = new Producto();
								Dt_Producto dts = new Dt_Producto();
								pr = dts.getProducto(Integer.parseInt(prID));						
                         	   %>
    ...
    <script>  
	  $(document).ready(function()
		{
			$("#nombreProducto").val("<%=pr.getProducto()%>");
			$("#descripcionProducto").val("<%=pr.getDescripcion()%>");	
			$("#cbxEstadoProducto").val("<%=pr.getEstadoproductoid()%>");	
			$("#cbxTipoProducto").val("<%=pr.getTipoproductoid()%>");	
		});
	 </script>  
